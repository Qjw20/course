...
parser.add_argument('--root_dir', default='/root/autodl-tmp/data/CCPD2019',type=str, help="root path of images and labels, include ./images and ./labels and classes.txt")
...
parser.add_argument('--split_by_file', default=True, help="define how to split the dataset, include ./train.txt ./val.txt ./test.txt ")
...
def yolo2coco(arg):
    ...
    #     root_path = os.path.join(images_pre_dir, name + "_1250")
    #     arg.root_dir = root_path
    #     print("Loading data from ",root_path)

    root_path = "/root/autodl-tmp/data/CCPD2019"
    ...
    for k, index in enumerate(tqdm(indexes)):
        ...
        if arg.random_split or arg.split_by_file:
            ...
            index_path = os.path.join(arg.root_dir,  "images/" + index + "\n")
            if index_path in train_img:             
                dataset = train_dataset
            elif index_path in val_img:
                dataset = val_dataset
            elif index_path in test_img:
    ...
    # 保存结果
    folder = os.path.join(images_pre_dir, 'annotations')
    if not os.path.exists(folder):
        os.makedirs(folder)
    if arg.random_split or arg.split_by_file:
        for phase in ['train','val','test']:
            json_name = os.path.join(images_pre_dir, 'annotations/{}.json'.format(phase))
        ...
        json_name = os.path.join(images_pre_dir, 'annotations/{}'.format(arg.save_path))
        with open(json_name, 'w') as f:
            json.dump(dataset, f)
            print('Save annotation to {}'.format(json_name))
