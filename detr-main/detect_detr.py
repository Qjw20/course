...
def plot_result(pil_img, prob, boxes, save_name=None, imshow=False, imwrite=True, save_crops=True, save_txt=True):
    ...
    im = cv2.imread('upload/{}'.format(save_name))

    if save_crops:
        if not os.path.exists("results/detect/crops"):
            os.makedirs('results/detect/crops')
        save_one_box(boxes, im, file='results/detect/crops/{}'.format(save_name), BGR=True)
    ...
    if save_txt:  # Write to file(txt)
        ...
        print("line:", line)
        txt_path = 'results/detect/labels/{}'.format(save_name).replace('jpg', 'txt')
...
if __name__ == "__main__":
    ...
    files = os.listdir("upload/") # <--修改为待预测图片所在文件夹路径
 
    cn = 0
    waste=0
    for file in files:
        img_path = os.path.join("upload/", file) # <--修改为待预测图片所在文件夹路径
        im = Image.open(img_path)

 
        scores, boxes, waste_time = detect(im, dfdetr, transform)
        plot_result(im, scores, boxes, save_name=file, imshow=False, imwrite=True, save_crops=True, save_txt=True)
