...
# plot box by opencv
def plot_result(pil_img, prob, boxes, files_dir, results_dir, save_name=None, imshow=False, imwrite=True, save_crops=True, save_txt=True):
    ...
    im = cv2.imread(files_dir + '{}'.format(save_name))

    if save_crops:
        if not os.path.exists(results_dir):
            os.makedirs(results_dir)
        save_one_box(boxes, im, file=results_dir + '/{}'.format(save_name), BGR=True)
    ...
    if save_txt:  # Write to file(txt)
        ...
        txt_path = (results_dir + '{}'.format(save_name).replace('jpg', 'txt')).replace('images', 'labels')
        if not os.path.exists(results_dir.replace('images', 'labels')):
            os.makedirs(results_dir.replace('images', 'labels'))
...
if __name__ == "__main__":
    ...
    files_pre_dir = "/root/autodl-tmp/data/CCPD2019/"
    results_pre_dir = "/root/autodl-tmp/data/CCPD2019/crops2017/"
    # sub_names = ['ccpd_base_1250', 'ccpd_blur_1250', 'ccpd_challenge_1250', 'ccpd_db_1250', 'ccpd_fn_1250', 'ccpd_rotate_1250', 'ccpd_tilt_1250', 'ccpd_weather_1250']
    sub_names = ['ccpd_challenge_1250', 'ccpd_db_1250', 'ccpd_fn_1250', 'ccpd_rotate_1250', 'ccpd_tilt_1250', 'ccpd_weather_1250']

    for name in sub_names:
        files_dir = files_pre_dir + name + "/val/"

        results_dir = results_pre_dir + name + "/images/"
        print(files_dir)
        files = os.listdir(files_dir) # <--修改为待预测图片所在文件夹路径
 
        cn = 0
        waste=0
        for file in files:
            img_path = os.path.join(files_dir, file) # <--修改为待预测图片所在文件夹路径
            im = Image.open(img_path)

    
            scores, boxes, waste_time = detect(im, dfdetr, transform)
            plot_result(im, scores, boxes, files_dir, results_dir, save_name=file, imshow=False, imwrite=False, save_crops=True, save_txt=True)
